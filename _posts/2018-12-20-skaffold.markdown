...
Pour intégrer jib dans skaffold, il faut 
...
Pour simplifier la structure des Jenkinsfile, nous avons intégré un Dockerfile au projet : 
...
Avant de faire ce choix, nous faisions du maven first. Dans ce contexte, Le Jenkinsfile appelle une commande maven qui en fonction de la stack sous jacente construit les livrables ou délègue la construction à d'autres outils plus adaptés (comme npm). 
...
Dans notre cas, nous définissons les tags via des variables d'environnement : 
